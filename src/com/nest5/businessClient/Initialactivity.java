...
public class Initialactivity extends FragmentActivity implements
	...
	// Validar si botÃƒÂ³n redime cupÃƒÂ³n o sello
	...
	private int totalSync = 0; //variable para contar cuantas synRows se han guardado con Ã©xito
					...
					runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							mResetProgressDialog
									.setMessage("Solicitando InformaciÃƒÂ³n al Servidor...");
	...
	protected void onStart() {
		super.onStart();
		//mReader.start();
		// If BT is not on, request that it be enabled.ï¿½ï¿½ï¿½ï¿½ï¿½ BTï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
        // setupChat() will then be called during onActivityRe//sultsetupChat() È»ï¿½ó½«µï¿½ï¿½ï¿½ï¿½Ú¼ï¿½ onActivityResult
	...
	public synchronized void onResume() {
		...
		//mReader.start();
		// Performing this check in onResume() covers the case in which BT wasï¿½ï¿½ onResume() ï¿½ï¿½Ö´ï¿½Ð´Ë¼ï¿½ï¿½ï¿½ï¿½ï¿½ BT ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
        // not enabled during onStart(), so we were paused to enable it...ï¿½ï¿½ onStart() ï¿½Ú¼ï¿½ï¿½Þ·ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ç±ï¿½ï¿½ï¿½Í£ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½...
        // onResume() will be called when ACTION_REQUEST_ENABLE activity returns.ACTION_REQUEST_ENABLE ï¿½î¶¯ï¿½ï¿½ï¿½ï¿½Ê±ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ onResume()ï¿½ï¿½
	...
	private void setHomeScreenContent() {
		...
		if (prefs.getInt(Util.INTERNET_CONNECTION, 0) == 1) {
			...
			// Toast.makeText(mContext, "No tienes conexiÃƒÂ³n a internet.",
			// Toast.LENGTH_LONG).show();
		}
	...
	private void createSyncRow(String table, long rowId,
			...
			if (syncRow != null) {
				if(isConnectedToInternet())
					sendAllSyncRows(); //sync todas las que haya, si esta es la unica, total serÃ¡ igual a uno y solo subirÃ¡ una, sino subirÃ¡ todas las que haya y habra un receiver para cada una.
			} else {
				Toast.makeText(mContext,
	...
	//checks if there are old unsynced rows and tries to sync them
		private void sendAllSyncRows() {
				...
				////Log.i("SYNC", "syncrows en db: "+String.valueOf(syncRows.size()));
				for(int i = 0; i < syncRows.size(); i++){//guardo cada row y si llega con Ã©xito resta uno
	...
	public class UploadFileToS3 extends AsyncTask<File, Void, Boolean> {
		...
		protected void onPostExecute(final Boolean status) {
			...
			if (status)
				...
				Toast.makeText(
						mContext,
						"Error guardando el archivo en la nube, intÃƒÂ©ntalo de nuevo por favor..",
						Toast.LENGTH_LONG).show();
		...
		public void handleMessage(Message msg) {
			...
			if (respuesta != null) {
				...
				if (status == 1) {
					...
					showMessageDialog("Tarjeta sellada con Ã‰xito",
			...
			if (respuesta != null) {
				...
				if (status == 201) { //se creo un objeto nuevo exitosamente, se debe actualizar el sync_id con el payload 
					...
					if(responsecode == 555 ){
						...
						if((sync_id != 0L) && (sync_row != 0L)){//se debe actualizar el valor en el objeto local porque fue creado como nuevo con Ã©xito en el servidor
					...
					if(status == 200){ //se actualizó una fila, 
						...
						if(responsecode == 555 ){
							...
							if((sync_row != 0L)){//se debe actualizar el valor en el objeto local porque fue creado como nuevo con Ã©xito en el servidor
								try{
									deleteSyncRow(sync_row);
		...
		public void handleMessage(Message msg) {
			...
			if (respuesta != null) {
				...
				if (status == 200) {
						...
						Toast.makeText(mContext, "No hay registros del dÃ­a para imprimir reporte", Toast.LENGTH_LONG).show();
					}
    ...
    private final Handler mHandlerBlueTooth = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case MESSAGE_WRITE:
                ...
                // construct a string from the bufferï¿½ï¿½ï¿½ï¿½Ò»ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ðµï¿½ï¿½Ö·ï¿½
            ...
            case MESSAGE_READ:
                ...
                // construct a string from the valid bytes in the bufferï¿½ï¿½ï¿½ï¿½Ò»ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ð§ï¿½Ö½Ú»ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ðµï¿½ï¿½Ö·ï¿½
                String readMessage = new String(readBuf, 0, msg.arg1);
                Log.d(TAG,"Leido: "+ readMessage);
                //mConversationArrayAdapter.add(mConnectedDeviceName+":  " + readMessage);
                break;
            case MESSAGE_DEVICE_NAME:
                // save the connected device's name ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½è±¸ï¿½ï¿½ï¿½
		...
		if (createdSale != null) {
			...
			while (it.hasNext()) {
				...
				createdSale.saveItem(dbHelper, pair.getKey().type,
						pair.getKey().id, pair.getValue());//serÃ¡ que acÃ¡ guarda el id local y sincroiniza asi por eso llega luego tod en ceros al volver a sincronizar?
